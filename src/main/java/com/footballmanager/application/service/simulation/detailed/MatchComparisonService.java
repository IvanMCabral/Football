...
/**
 * {@link MatchComparison} for a finished detailed match by replaying the
 ...
 *   <li>Build a fresh {@code MatchContext} by applying each persisted
 *       sub in order to {@code baseline.initialContext()} via
 *       {@link MatchContext#withManualSubstitution}. The F2.5 design
 ...
 * </ol>
 *
 * <p>Why standalone (not via {@code LiveSession}):
 * <ul>
...
public class MatchComparisonService {
    ...
    private final DetailedMatchStoragePort detailStoragePort;
    private final BaselineStateStoragePort baselineStoragePort;

    public MatchComparisonService(
            DetailedMatchStoragePort detailStoragePort,
     ...
     * @throws LiveDetailNotFoundException if no detailed match detail exists for the match
     *         (match hasn't finished yet, or the detailed match path was disabled)
     */
    /**
    ...
    public Mono<MatchComparison> getComparison(String careerId, String matchId) {
        ...
        Mono<DetailedMatchData> liveMono = detailStoragePort.findByMatchId(fCareerId, fMatchId)
        ...
        return liveMono.flatMap(live ->
                baselineMono.flatMap(baseline -> {
                    ...
                    MatchContext ctx = baseline.initialContext();
                    ...
                    final MatchContext ctxFinal = ctx;
                    final DetailedMatchData liveFinal = live;
                    return Mono.fromCallable(() -> {
                        ...
                        DetailedMatchResult baselineResult = engine.simulate(ctxFinal, random);
                        ...
                        DetailedMatchData baselineData = DetailedMatchData.fromResult(
                                careerId,
                                ...
                                awayFormation,
                                baselineResult,
                                Collections.<PlayerMatchRatingDto>emptyList());
     ...
     * detailed match path was disabled for the career.
     */
    public static class LiveDetailNotFoundException extends RuntimeException {
        public LiveDetailNotFoundException(String careerId, String matchId) {
            super("Live detailed match detail not found for careerId=" + careerId + ", matchId=" + matchId);
        }
    }
