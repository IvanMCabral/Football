...
public final class TimelineSnapshotBuilder {
     ...
     */
    public static TimelineSnapshot build(DetailedMatchData detail, int minute) {
        ...
        List<DetailedMatchEventDto> events = detail.timeline();
        List<DetailedMatchEventDto> filtered = new ArrayList<>(events.size());
        int homeGoals = 0;
        int awayGoals = 0;
        int homeShots = 0;
        int awayShots = 0;
        ...
        for (DetailedMatchEventDto e : events) {
            if (e.minute() > minute) {
                continue;
            }
            filtered.add(e);
