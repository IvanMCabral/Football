...
/**
 *  Pure function discipline/foul card model for detailed match engine.
 *
 * <p>Provides:
 * <ul>
 ...
 */
public final class DisciplineModel {
     ...
     */
    public double foulProbability(PlayerMatchState player, TeamStyle style, boolean defending) {
        if (player == null) {
        ...
        double styleMod = switch (style) {
            ...
            case WIDE_PLAY, LEFT_FLANK, RIGHT_FLANK, CENTRAL_PLAY -> 0.000;
            case BALANCED -> 0.000;
        };
     ...
     */
    public double yellowCardProbability(PlayerMatchState player, TeamStyle style) {
        if (player == null) {
            throw new IllegalArgumentException("player must not be null");
        }
        double base = 0.40;
     ...
     */
    public boolean shouldCommitFoul(PlayerMatchState player, TeamStyle style,
     ...
     */
    public boolean shouldReceiveYellow(PlayerMatchState player, TeamStyle style, Random random) {
        if (player == null) {
            throw new IllegalArgumentException("player must not be null");
        }
        if (random == null) {
     ...
     */
    public boolean shouldProduceRedCard(PlayerMatchState player) {
        if (player == null) {
            throw new IllegalArgumentException("player must not be null");
        }
     ...
     */
    public boolean isRedCarded(PlayerMatchState player) {
        if (player == null) {
            throw new IllegalArgumentException("player must not be null");
        }
