...
public class PlayerSeasonStatsQueryService {
    ...
    public Mono<PlayerSeasonStatsResponse> getPlayerSeasonStats(
        ...
        if (!apiEnabled) {
            return Mono.just(PlayerSeasonStatsResponse.builder()
                    ...
                    .season(season)
                    .incomplete(true)
                    .message("Player season stats require detailed match detail API to be enabled")
                    .build());
        }
    ...
    private PlayerSeasonStatsResponse buildResponse(
            ...
            String sortBy,
            String order,
            List<DetailedMatchData> details) {

        log.info("[DETAIL-STATS-QUERY] careerId={}, teamId={}, details.size={}", careerId, teamId, details.size());

        for (int i = 0; i < details.size(); i++) {
            DetailedMatchData d = details.get(i);
        ...
        var seasonNumbers = details.stream()
                .map(DetailedMatchData::seasonNumber)
                .filter(java.util.Objects::nonNull)
                .distinct()
                .toList();
        log.info("[DETAIL-STATS-SEASON] season param={}, distinct seasonNumbers in details={}", season, seasonNumbers);

        List<DetailedMatchData> seasonDetails = details.stream()
                .filter(d -> d.seasonNumber() != null && d.seasonNumber().equals(season))
                .collect(Collectors.toList());

        log.info("[DETAIL-STATS-SEASON] seasonDetails.size={} after filter with season={}", seasonDetails.size(), season);
        ...
        if (seasonDetails.isEmpty()) {
            return PlayerSeasonStatsResponse.builder()
                    ...
                    .averageRating(0.0)
                    .incomplete(false)
                    .message("No detailed match detail data found for career " + careerId + " season " + season)
                    .build();
        }
        ...
        log.info("[DETAIL-STATS-AGG] playerStatsSize={}, totalMatchesProcessed={}, matchIds={}, teamId={}",
    ...
    private List<PlayerSeasonStatsWarning> buildWarnings(AggregationResult result) {
        ...
        if (result.matchIds().isEmpty()) {
            warnings.add(new PlayerSeasonStatsWarning(
                    ...
                    "No detailed match detail data found for this career/season",
                    null));
        }
