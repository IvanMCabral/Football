 ...
 * <ul>
 *   <li>Only starting XI players appear in playerRatings (PlayerRatingsAssembler limitation)</li>
...
public final class PlayerSeasonStatsAggregator {
    ...
    public PlayerSeasonStatsResponse aggregate(
        ...
        Map<String, DetailedMatchData> deduped = new HashMap<>();
        for (DetailedMatchData detail : details) {
        ...
        for (DetailedMatchData detail : deduped.values()) {
            if (detail.playerRatings() == null) continue;
            for (PlayerMatchRatingDto rating : detail.playerRatings()) {
    ...
    public AggregationResult aggregateWithMetadata(
        ...
        int lastUpdatedRound = deduped.values().stream()
                .filter(d -> d.round() > 0)
                .mapToInt(DetailedMatchData::round)
                .max().orElse(0);
    ...
    private static final class PlayerAccumulator {
        ...
        void addMatch(int round, PlayerMatchRatingDto r) {
            appearances++;
