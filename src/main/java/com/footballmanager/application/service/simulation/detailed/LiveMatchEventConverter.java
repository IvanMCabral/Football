...
/**
 *
 * <p>Converts entity.DetailedMatchEvent (with playerName, no playerId) to DetailedMatchEvent
 ...
 */
@Slf4j
public final class LiveMatchEventConverter {

    private final CareerSave career;
    private final String homeTeamId;
    private final String awayTeamId;

    public LiveMatchEventConverter(CareerSave career, String homeTeamId, String awayTeamId) {
    ...
    /**
     * Convert a single live entity.DetailedMatchEvent to DetailedMatchEvent.
     */
    public DetailedMatchEvent convert(MatchEvent liveEvent) {
        if (liveEvent == null) {
            return null;
        }
    ...
    /**
     * Build MatchTimeline from list of live entity.DetailedMatchEvent.
     */
    public MatchTimeline buildTimeline(List<MatchEvent> liveEvents) {
    ...
    /**
     * Map entity.DetailedMatchEventType to DetailedMatchEventType.
     * Every domain type maps explicitly.
     */
    private DetailedMatchEventType mapEventType(MatchEvent liveEvent) {
        MatchEvent.EventType type = liveEvent.getEventType();
        if (type == null) {
            throw new IllegalArgumentException("EventType cannot be null");
        }
        return switch (type) {
            case GOAL -> DetailedMatchEventType.GOAL;
            case SHOT -> DetailedMatchEventType.SHOT;
            case SHOT_ON_TARGET -> DetailedMatchEventType.SHOT_ON_TARGET;
            case SAVE -> DetailedMatchEventType.SAVE;
            case MISS -> DetailedMatchEventType.MISS;
            case BLOCK -> DetailedMatchEventType.BLOCK;
            case CHANCE_CREATED -> DetailedMatchEventType.CHANCE_CREATED;
            case FOUL -> DetailedMatchEventType.FOUL;
            case YELLOW_CARD -> DetailedMatchEventType.YELLOW_CARD;
            case RED_CARD -> DetailedMatchEventType.RED_CARD;
            case INJURY -> DetailedMatchEventType.INJURY;
            case CORNER -> DetailedMatchEventType.CORNER;
            case OFFSIDE -> DetailedMatchEventType.OFFSIDE;
            case SUBSTITUTION -> DetailedMatchEventType.SUBSTITUTION;
            case TACTICAL_CHANGE -> DetailedMatchEventType.TACTICAL_CHANGE;
            case CARD -> {
                String desc = liveEvent.getDescription() != null ? liveEvent.getDescription().toLowerCase() : "";
                if (desc.contains("red") || desc.contains("roja")) {
                    yield DetailedMatchEventType.RED_CARD;
                }
                yield DetailedMatchEventType.YELLOW_CARD;
            }
        };
    }
