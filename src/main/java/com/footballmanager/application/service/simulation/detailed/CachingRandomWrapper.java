 ...
 * user mutates the match state via {@link LiveSession#mutateContext}.
 *
 * <p><b>Why we need this</b>: the detailed match engine consumes doubles from THREE
...
public class CachingRandomWrapper extends Random {
     ...
     * previous engine run; the caller (LiveSession.tick) calls
     ...
     *
     * <p>Used by {@code LiveSession.replayFromMinute(int minute)} to tell
    ...
    public synchronized void invalidateFromIndex(int index) {
        ...
        // (only inside PlayerSelector) so the loss is bounded.
    ...
    // the detailed match engine does not call them. If that changes, the F1 prompt
    ...
    // ========== Read-only accessors (for tests + B3 LiveSession) ==========
