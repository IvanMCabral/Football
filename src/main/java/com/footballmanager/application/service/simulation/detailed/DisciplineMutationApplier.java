 ...
 * </ul>
 */
public class DisciplineMutationApplier {
     ...
     */
    public int applyDiscipline(CareerSave career, DetailedMatchResult result,
        ...
        for (DetailedMatchEvent event : result.timeline().events()) {
            if (event.type() == DetailedMatchEventType.RED_CARD && event.playerId() != null
        ...
        for (DetailedMatchEvent event : result.timeline().events()) {
            DetailedMatchEventType type = event.type();
            String playerId = event.playerId();

            if (playerId == null || playerId.isBlank()) continue;

            if (type == DetailedMatchEventType.YELLOW_CARD) {
            ...
            } else if (type == DetailedMatchEventType.RED_CARD) {
