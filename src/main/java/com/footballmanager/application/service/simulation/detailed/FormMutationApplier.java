 ...
 * from detailed player match ratings.
 *
 ...
 *   <li>Starting XI players only (from PlayerRatingsAssembler output)</li>
 * </ul>
 */
public final class FormMutationApplier {
    ...
    /**
     * Apply form updates from detailed player match ratings to CareerSave SessionPlayers.
     *
     ...
     */
    public int applyForm(CareerSave career, DetailedMatchResult result, CareerMutationPolicy policy) {
        ...
        List<PlayerMatchRatingDto> ratings = assembleRatings(career, result);
     ...
     * Uses PlayerMatchStatsModel directly (no modification to PlayerRatingsAssembler needed).
     */
    private List<PlayerMatchRatingDto> assembleRatings(CareerSave career, DetailedMatchResult result) {
        // Resolve starting XI using career playerManager (not teamStarting11)
        java.util.List<PlayerMatchState> allPlayers = new java.util.ArrayList<>();
        ...
        for (String teamId : java.util.List.of(result.homeTeamId(), result.awayTeamId())) {
            ...
            for (String playerId : starterIds) {
                ...
                if (sp != null) {
                    allPlayers.add(PlayerMatchState.fromSessionPlayer(sp, teamId));
                }
            }
        }
        ...
        // Use PlayerMatchStatsModel directly — package-visible, no changes to assembler needed
