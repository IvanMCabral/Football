...
/**
 *  Substitution engine for detailed match engine.
 *
 * <p>Provides:
 * <ul>
...
public final class SubstitutionEngine {
    ...
    private static final Logger log = LoggerFactory.getLogger(SubstitutionEngine.class);
    ...
    public SubstitutionEngine(int maxSubstitutions) {
    ...
    public Optional<DetailedMatchEvent> attemptSubstitution(TeamMatchState team, int minute) {
        if (team == null) {
    ...
    private Optional<DetailedMatchEvent> makeSubstitution(TeamMatchState team, PlayerMatchState subOff, int minute) {
        String teamId = team.teamId();
        ...
        if (!formationHasSlotForPosition(team.formation(), subOff.position())) {
            log.trace("SubstitutionEngine.attemptSubstitution: "
                + "current formation '{}' has no slot for position '{}' on team {} — skipping",
                team.formation(), subOff.position(), teamId);
            return Optional.empty();
        }

        PlayerMatchState subOn = selectBenchPlayer(team.benchPlayers(), subOff.position());
        if (subOn == null) {
            return Optional.empty();
        }
        ...
        return Optional.of(new DetailedMatchEvent(
                minute,
     ...
     *
     * <p>Implementation: parse the formation with {@link FormationParser}
    ...
    private boolean formationHasSlotForPosition(String formation, String position) {
        ...
        FormationParser.FormationShape parsed = new FormationParser().parse(formation);
    ...
    private Optional<PlayerMatchState> findCandidate(List<PlayerMatchState> players,
                                                         java.util.function.Predicate<PlayerMatchState> filter) {
    ...
    private boolean isGoalkeeper(PlayerMatchState player) {
        return player != null && "GK".equals(player.position());
    }

    private PlayerMatchState selectBenchPlayer(List<PlayerMatchState> bench, String position) {
     ...
     * The {@code LiveSession} caches events as the engine ticks through its
     ...
     * @return the {@link DetailedMatchEvent} recording the substitution
     ...
     */
    public DetailedMatchEvent manualSubstitute(TeamMatchState team,
                                          String playerOffId,
                                          String playerOnId,
                                          int minute) {
        if (team == null) {
        ...
        PlayerMatchState subOff = team.startingPlayers().stream()
        ...
        // An injured player is represented as off-pitch by PlayerMatchState.injure().
        ...
        PlayerMatchState subOn = team.benchPlayers().stream()
