...
final class ShotLocationService {

    private static final ShotLocation[] LOCATIONS = ShotLocation.values();
    private static final FormationParser FORMATION_PARSER = new FormationParser();

    private final ShotCoordinateGenerator coordGenerator = new ShotCoordinateGenerator();

    ShotLocation selectShotLocation(
            TeamStyle style,
            String formation,
        ...
        if (total <= 0.0) {
            return ShotLocation.PENALTY_AREA_CENTER;
        }
    ...
    ShotCoordinate generateShotCoordinate(
    ...
    private ShotCoordinate generateShotCoordinate(ShotLocation location, TeamStyle style, Random random) {
        if (location == ShotLocation.PENALTY_AREA_WIDE && style == TeamStyle.LEFT_FLANK) {
            return coordGenerator.generateWideFlank(true, random);
        }
        if (location == ShotLocation.PENALTY_AREA_WIDE && style == TeamStyle.RIGHT_FLANK) {
    ...
    private void applyFormationShapeShift(double[] weights, String formation) {
        FormationParser.FormationShape parsed = FORMATION_PARSER.parse(formation);
