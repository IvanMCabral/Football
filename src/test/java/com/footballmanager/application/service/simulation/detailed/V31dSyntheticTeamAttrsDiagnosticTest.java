 ...
 *       <td>Hunt the multiplier in classic MatchEngineImpl, LiveSession, LeagueSimulator, CareerSimulator</td></tr>
 * </table>
 *
 * <p><b>Usage:</b>
 * <pre>
...
class V31dSyntheticTeamAttrsDiagnosticTest {
    ...
    void synthetic_12_team_league_runtime_replica() {
        ...
        if (Math.abs(conversionRate - RUNTIME_CONVERSION_RATE) < 0.02) {
            ...
            System.out.println("      Phase 2: hunt multiplier in classic MatchEngineImpl, LiveSession, LeagueSimulator, CareerSimulator.");
        } else {
    ...
    private static final class PerMatchMetrics {
        ...
        void add(DetailedMatchResult r) {
            matchCount++;
            ...
            for (DetailedMatchEvent shot : r.timeline().shotEvents()) {
    ...
    private PerMatchMetrics runMatches(DetailedMatchEngine engine, SyntheticTeam homeTeam,
        ...
        for (int i = 0; i < nPerPairing; i++) {
            ...
            MatchContext ctx = buildContext(homeTeam, opp, i + 1);
            DetailedMatchResult result = engine.simulate(ctx, i + 1);
            metrics.add(result);
        }
        return metrics;
    }

    private MatchContext buildContext(SyntheticTeam home, SyntheticTeam away, int seed) {
