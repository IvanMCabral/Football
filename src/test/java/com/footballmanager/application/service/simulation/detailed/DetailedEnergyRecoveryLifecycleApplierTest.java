 ...
 */
class EnergyRecoveryLifecycleApplierTest {

    private static final int RECOVERY = 8;
    private static final int CAP = 100;

    private final EnergyRecoveryLifecycleApplier applier = new EnergyRecoveryLifecycleApplier();

    // ========== Helper builders ==========

    private CareerMutationPolicy policy(boolean enabled) {
        return new CareerMutationPolicy(enabled, enabled, enabled, enabled, enabled);
    }
    ...
    void fatiguePersistenceFalse_doesNothing() {
        CareerSave career = careerWithTeamAndPlayer("team-A", "p1",
                makePlayer("p1", 70, false));
        CareerMutationPolicy pol = policy(true, true, false, true, true);
    ...
    void mutateCareerStateFalse_doesNothing() {
        CareerSave career = careerWithTeamAndPlayer("team-A", "p1",
                makePlayer("p1", 70, false));
        CareerMutationPolicy pol = policy(false, true, true, true, true);
        int result = applier.applyRecovery(career, set(), pol);
        assertEquals(0, result);
        assertEquals(70, career.getSessionPlayer("p1").getEnergy());
    }
