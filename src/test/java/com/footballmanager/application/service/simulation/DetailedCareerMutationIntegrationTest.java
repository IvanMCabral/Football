...
class DetailedCareerMutationIntegrationTest {
    ...
    void noInjuryEvents_noMutation() {
        ...
        // detailed match engine ran; if no INJURY events in timeline, no mutation
    ...
    void mutationAfterDetailedSuccess_only() {
        ...
        // detailed match path completed; mutation was attempted but may or may not have applied
        // depending on whether detailed match engine generated INJURY events
    }
     ...
     * Regression: LeagueSimulator.applyDetailedCareerMutation checks isCareerMutationEnabled() first.
     */
    @Test
    ...
    void persistDisciplineEnabled_appliesDisciplineMutation() {
        ...
        // (card events are produced by detailed match engine naturally)
    }
    ...
    void persistDisciplineIndependentFromInjuryAndFatigue() {
        ...
        // Round completes; discipline may or may not be applied depending on detailed match engine events
    }
    ...
    void detailedDisabledWithDisciplineFlags_noMutation() {
        ...
        assertEquals(originalYellow, p.getYellowCards(),
                "Yellow cards should not change in classic path");
        assertFalse(p.getSuspended(),
                "Player should not be suspended in classic path");
    }
     ...
     * Fake detailed match result has no events for suspended player.
    ...
    void preExistingSuspension_notParticipated_decrementsAndClears() {
        ...
        // Fake detailed match result with no RED_CARD, no events for suspended player
        DetailedMatchResult fakeResult = DetailedMatchResult.builder()
                ...
                .timeline(new com.footballmanager.application.service.simulation.detailed.MatchTimeline())
     ...
     * Fake detailed match result with no RED_CARD for suspended player.
     * Suspended player is NOT in starting XI, NOT in timeline.
     ...
     * Fake detailed match result includes RED_CARD for the player.
    ...
    void redCardInRound_playerStillSuspendedAfterRound() {
        ...
        com.footballmanager.application.service.simulation.detailed.MatchTimeline timeline =
                new com.footballmanager.application.service.simulation.detailed.MatchTimeline();
        timeline.addEvent(new com.footballmanager.application.service.simulation.detailed.DetailedMatchEvent(
                45,
                com.footballmanager.application.service.simulation.detailed.DetailedMatchEventType.RED_CARD,
    ...
    void detailedDisabled_noLifecycleChange() {
        ...
        LeagueSimulator simulator = new LeagueSimulator(
                fakeSim, null, false, false, false, fakeStorage,
                true, false, false, true, false,
                (ctx, seed) -> { throw new AssertionError("detailed match engine should not be used"); });
     ...
     *
     * Player starts with yellowCards=4. detailed match engine emits one YELLOW_CARD → threshold triggers.
    ...
    void thresholdSuspendedPlayer_notDecrementedSameRound() {
        ...
        timeline.addEvent(new com.footballmanager.application.service.simulation.detailed.DetailedMatchEvent(
                30,
                com.footballmanager.application.service.simulation.detailed.DetailedMatchEventType.YELLOW_CARD,
    ...
    void detailedDisabled_noThresholdEffect() {
        ...
        SessionPlayer p = career.getSessionPlayer("p1");
        assertEquals(4, p.getYellowCards(), "Yellow cards should not change in classic path");
        assertFalse(p.getSuspended(), "Player should not be suspended in classic path");
    }
    ...
    void persistFormEnabled_appliesFormMutation() {
        ...
        timeline.addEvent(new com.footballmanager.application.service.simulation.detailed.DetailedMatchEvent(
                30,
                com.footballmanager.application.service.simulation.detailed.DetailedMatchEventType.GOAL,
    ...
    void detailedDisabledWithFormFlags_noMutation() {
        ...
        assertTrue(fakeSim.simulateQuickCalled, "Default path should be used");
        SessionPlayer p = career.getSessionPlayer("p1-form");
        assertEquals(50, p.getForm(), "Form should not change in classic path");
    }
    ...
    void injuryRecovery_newlyInjured_doesNotDecrementSameRound() {
        ...
        // Fake detailed match result with INJURY event for injured_p1
        com.footballmanager.application.service.simulation.detailed.MatchTimeline timeline =
                new com.footballmanager.application.service.simulation.detailed.MatchTimeline();
        timeline.addEvent(new com.footballmanager.application.service.simulation.detailed.DetailedMatchEvent(
                30,
                com.footballmanager.application.service.simulation.detailed.DetailedMatchEventType.INJURY,
    ...
    void injuryRecovery_detailedDisabled_noOp() {
        ...
        SessionPlayer p = career.getSessionPlayer("injured_p1");
        assertTrue(p.getInjured(), "Player should remain injured (detailed match path not used)");
        assertEquals(1, p.getInjuryRemainingMatches(), "Remaining should not change");
    }
    ...
    void energyRecovery_participatingPlayerDrainsAndDoesNotReceiveRecovery() {
        ...
        // GOAL event: player participates, energy drains to 88 (FatigueMutationApplier).
    ...
    private static CareerSave makeCareerWithEnergyPlayer(
        ...
        // Create all starters with their expected pids (for MatchContext resolve)
        List<SessionPlayer> homePlayers = new ArrayList<>();
        for (int i = 0; i < homeStarterCount; i++) {
            String expectedPid = "p_" + homeTeamId + "_" + i;
            SessionPlayer p = SessionPlayer.custom(expectedPid, 25, "MID",
    ...
    private static CareerSave makeCareerWithSuspension(
        ...
        for (int i = 0; i < homeStarterCount; i++) {
            ...
            SessionPlayer p = SessionPlayer.custom(expectedPid, 25, "MID",
                    75, 75, 75, 75, 75, 75, BigDecimal.valueOf(1000));
            // Force sessionPlayerId to expected pid so MatchContext can resolve
            p.setSessionPlayerId(expectedPid);
            pm.addSessionPlayer(p);
            tm.assignPlayerToSquad(expectedPid, homeTeamId);
            homePlayers.add(p);
    ...
    private static class FakeStoragePort implements DetailedMatchStoragePort {
        ...
        DetailedMatchData savedDetail = null;

        @Override
        public Mono<Void> save(String careerId, DetailedMatchData detail) {this.saveCalled = true;
    ...
    private static class ThrowingStoragePort implements DetailedMatchStoragePort {
        @Override
        public Mono<Void> save(String careerId, DetailedMatchData detail) { return Mono.error(new RuntimeException("Simulated storage failure")); }
