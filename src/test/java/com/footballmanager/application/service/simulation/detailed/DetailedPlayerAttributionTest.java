...
class DetailedPlayerAttributionTest {
    ...
    void noSyntheticPlayerIdsInEvents() {
        MatchContext ctx = buildContext("real-ids-1", 75, 75);
        DetailedMatchEngine engine = new DetailedMatchEngine();
        DetailedMatchResult r = engine.simulate(ctx, 12345L);
    ...
    void playerNamesAreRealFromSessionPlayer() {
        MatchContext ctx = buildContext("real-names-1", 75, 75);
        DetailedMatchEngine engine = new DetailedMatchEngine();
        DetailedMatchResult r = engine.simulate(ctx, 54321L);
    ...
    void goalEventsHaveRealAttribution() {
        MatchContext ctx = buildContext("goal-attr-1", 80, 75);
        DetailedMatchEngine engine = new DetailedMatchEngine();
        DetailedMatchResult r = engine.simulate(ctx, 98765L);

        for (DetailedMatchEvent e : r.timeline().goalEvents()) {
            ...
            // Note: if player.getName() returns null/blank, PlayerMatchState
    ...
    void shotEventsHaveNonNullPlayerIds() {
        MatchContext ctx = buildContext("shot-ids-1", 75, 75);
        DetailedMatchEngine engine = new DetailedMatchEngine();
        DetailedMatchResult r = engine.simulate(ctx, 11111L);

        for (DetailedMatchEvent e : r.timeline().shotEvents()) {
    ...
    void shooterAndAssistAreDifferentPlayers() {
        MatchContext ctx = buildContext("shooter-assist-1", 80, 80);
        DetailedMatchEngine engine = new DetailedMatchEngine();
        DetailedMatchResult r = engine.simulate(ctx, 22222L);

        List<DetailedMatchEvent> shotEvents = r.timeline().shotEvents();
    ...
    void homeAndAwayPlayersAreDistinguishable() {
        MatchContext ctx = buildContext("home-away-1", 75, 75);
        DetailedMatchEngine engine = new DetailedMatchEngine();
        DetailedMatchResult r = engine.simulate(ctx, 33333L);
        ...
        List<DetailedMatchEvent> homeEvents = r.timeline().events().stream()
                .filter(e -> homeId.equals(e.teamId()))
                .toList();
        List<DetailedMatchEvent> awayEvents = r.timeline().events().stream()
    ...
    void substituteEventsHaveBothPlayerIds() {
        MatchContext ctx = buildContext("sub-event-1", 80, 80);
        DetailedMatchEngine engine = new DetailedMatchEngine();
        DetailedMatchResult r = engine.simulate(ctx, 44444L);

        List<DetailedMatchEvent> subEvents = r.timeline().events().stream()
                .filter(e -> e.type() == DetailedMatchEventType.SUBSTITUTION)
                .toList();

        for (DetailedMatchEvent e : subEvents) {
    ...
    void allPlayerIdsFromStartingOrBenchPlayers() {
        MatchContext ctx = buildContext("player-pool-1", 75, 75);
        DetailedMatchEngine engine = new DetailedMatchEngine();
        DetailedMatchResult r = engine.simulate(ctx, 55555L);
