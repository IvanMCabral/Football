...
class V29dPerPositionAttributesDiagnosticTest {
    ...
    private void runScenario(TeamStyle style, String label, int homeOvr, int awayOvr) {
        ...
        for (int seed = 1; seed <= N_SIMULATIONS; seed++) {
            MatchContext ctx = buildContext(style, "v29d-" + style + "-" + label + "-" + seed,
                    homeOvr, awayOvr);
    ...
    private double runAndAverage(DetailedMatchEngine engine, TeamStyle style, int homeOvr, int awayOvr) {
        long totalGoals = 0;
        for (int seed = 1; seed <= N_SIMULATIONS; seed++) {
            MatchContext ctx = buildContext(style, "v29d-summary-" + style + "-" + homeOvr + "x" + awayOvr + "-" + seed,
                    homeOvr, awayOvr);
            DetailedMatchResult result = engine.simulate(ctx, seed);
            totalGoals += result.homeGoals() + result.awayGoals();
        }
