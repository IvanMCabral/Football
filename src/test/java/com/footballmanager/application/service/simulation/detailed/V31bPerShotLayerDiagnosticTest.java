 ...
 * data via MatchTimeline. The summary test aggregates per layer and identifies
...
class V31bPerShotLayerDiagnosticTest {
    ...
    private static final class PerLayerAccumulator {
        ...
        EnumMap<ShotLocation, Long> shotLocationCounts = new EnumMap<>(ShotLocation.class);
        EnumMap<ShotLocation, Long> shotLocationGoals = new EnumMap<>(ShotLocation.class);
        ...
        void attributeGapToLayers(PerLayerAccumulator des, double runtimeGoals, double diagGoals) {
            ...
            long longRange = shotLocationCounts.get(ShotLocation.LONG_RANGE);
    ...
    private void runScenario(TeamStyle style, String label, int homeOvr, int awayOvr) {
        ...
        for (int seed = 1; seed <= N_SIMULATIONS; seed++) {
            MatchContext ctx = buildContext(style, "v31b-" + style + "-" + label + "-" + seed,
                    homeOvr, awayOvr);
            ...
            for (DetailedMatchEvent shot : shotEvents) {
                if (shot.shotCoordinate() != null) {
                    ShotLocation loc = shot.shotCoordinate().location();
            ...
            for (DetailedMatchEvent goal : goalEvents) {
                if (goal.shotCoordinate() != null) {
                    ShotLocation loc = goal.shotCoordinate().location();
    ...
    private PerLayerAccumulator runLayerAccumulator(DetailedMatchEngine engine, TeamStyle style,
        ...
        for (int seed = 1; seed <= N_SIMULATIONS; seed++) {
            MatchContext ctx = buildContext(style, "v31b-phase1b-" + label + "-" + seed,
                    homeOvr, awayOvr);
            DetailedMatchResult result = engine.simulate(ctx, seed);
    ...
    private MatchContext buildContext(TeamStyle style, String matchId, int homeOvr, int awayOvr) {
