...
import com.footballmanager.application.service.simulation.DetailedSeasonShapeDiagnosticSupport.SeasonShapeContext;
...
class DetailedMutationBalancingDiagnosticTest {
    ...
    void diagnostic_syntheticInvariant_preservesMutationBounds() {
        ...
        LeagueSimulator simulator = new LeagueSimulator(
                fakeSim, null,
                false,  // useClassicLeagueEngine
                true,   // useDetailedMatchEngine
                false,  // persistDetail
                fakeStorage,
    ...
    /**
     * Real detailed match engine stress diagnostic — WORST-CASE no-rotation.
     *
    ...
    void diagnostic_realDetailedEngine_fixedXIStress() {
        ...
        if (totalInjuriesObserved == 0) {
            ...
            System.out.println("  2. DetailedMatchEngine may call injury model less frequently than once per player per match");
        ...
        if (totalYellows == 0 && totalReds == 0) {
            ...
            System.out.println("  Possible reasons:");
            System.out.println("  1. DisciplineModel may not emit card events in current implementation");
            System.out.println("  2. Card events may require specific match situations to trigger");
            System.out.println("  3. Discipline mutation may not be wired into DetailedMatchEngine timeline");
    ...
    /**
     * Real detailed match engine rotation-aware diagnostic — PRIMARY BALANCING MEASUREMENT.
     *
    ...
    void diagnostic_seasonShaped_realDetailedEngine_baselineMetricsForReview() {
        ...
        SeasonShapeContext ctx = DetailedSeasonShapeDiagnosticSupport.createSeasonShapeContext(8, 25, 30);
        CareerSave career = ctx.career();

        // Initialize starting XI for all teams
        for (String teamId : ctx.teamIds()) {
            DetailedSeasonShapeDiagnosticSupport.selectStartingXI(career, teamId, 11);
        }
        ...
        for (int round = 1; round <= 30; round++) {
            ...
            for (String teamId : ctx.teamIds()) {
                preInjured.put(teamId, DetailedSeasonShapeDiagnosticSupport.getInjuredIds(career, teamId));
                preSuspended.put(teamId, DetailedSeasonShapeDiagnosticSupport.getSuspendedIds(career, teamId));
            }
            ...
            // Post-round analysis
            for (String teamId : ctx.teamIds()) {
                Set<String> postInjured = DetailedSeasonShapeDiagnosticSupport.getInjuredIds(career, teamId);
                Set<String> postSuspended = DetailedSeasonShapeDiagnosticSupport.getSuspendedIds(career, teamId);
                ...
                // Yellow/red cards
                teamYellowCards.put(teamId, DetailedSeasonShapeDiagnosticSupport.countTeamYellowCards(career, teamId));
                teamRedCards.put(teamId, DetailedSeasonShapeDiagnosticSupport.countTeamRedCards(career, teamId));

                // Per-round snapshots
                teamEnergyByRound.get(teamId).add(DetailedSeasonShapeDiagnosticSupport.computeSquadAvgEnergy(career, teamId));
                teamUnavailableByRound.get(teamId).add(DetailedSeasonShapeDiagnosticSupport.countUniqueUnavailable(career, teamId));
                ...
                if (starters != null) {
                    for (String sid : starters) {
                        ...
                        if (p != null && !DetailedSeasonShapeDiagnosticSupport.isPlayerAvailable(p)) {
                ...
                if (starters != null) {
                    for (String sid : starters) {
                        ...
                        if (p != null && p.getEnergy() <= 20 &&
                                DetailedSeasonShapeDiagnosticSupport.isPlayerAvailable(p)) {
                ...
                totalRotations += DetailedSeasonShapeDiagnosticSupport.rotateDueToEnergy(career, teamId, 30, 15);
            }
        }
        ...
        for (String teamId : ctx.teamIds()) {
            ...
            leagueAvgFormEnd += DetailedSeasonShapeDiagnosticSupport.computeTeamAvgForm(career, teamId);
            int tMinForm = DetailedSeasonShapeDiagnosticSupport.computeTeamMinForm(career, teamId);
            int tMaxForm = DetailedSeasonShapeDiagnosticSupport.computeTeamMaxForm(career, teamId);
    ...
    private static class FakeStoragePort implements DetailedMatchStoragePort {
        ...
        @Override
        public Mono<Void> save(String careerId, DetailedMatchData detail) {saveCalled = true; return Mono.empty();}
     ...
     */
    private static class MinimalDeterministicEngine implements DetailedMatchEngineProvider {
