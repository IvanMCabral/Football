...
class MatchComparisonDiffTest {
    ...
    void calculate_emits90EntriesFor5TypesAcross18Buckets() {
        DetailedMatchData baseline = sampleData(1, 0, 1.0, 0.5, 8, 5, 50);
        DetailedMatchData live = sampleData(2, 1, 1.8, 0.9, 12, 8, 55);
    ...
    void calculate_emptyTimelines_yieldsAllZeros() {
        DetailedMatchData baseline = sampleData(0, 0, 0.0, 0.0, 0, 0, 50);
        DetailedMatchData live = sampleData(0, 0, 0.0, 0.0, 0, 0, 50);
    ...
    void calculate_eventsInCorrectBucket() {
        ...
        DetailedMatchData baseline = sampleDataWithEvents(
                List.of(eventAt(22, "GOAL"))
        );
        DetailedMatchData live = sampleDataWithEvents(
    ...
    private DetailedMatchEventDto eventAt(int minute, String type) {
        return new DetailedMatchEventDto(
                minute, type, "home-id", "player-1", "Player One",
                null, null, 0.0, "Test event at minute " + minute, null);
    }

    private DetailedMatchData sampleData(int homeGoals, int awayGoals,
