...
/**
 *
 * {@link ShotXgCalculator} that over-corrected:
 * <ol>
...
class V33CalibrationDiagnosticTest {
    ...
    void v33aRegressionGuard() {
        ...
        System.out.println("(cap=2.5, statsAmp=0.025 hardcoded in ShotXgCalculator)");
        System.out.println("================================================================");
        ...
        assertTrue(intermediosAvg >= 0.9 && intermediosAvg <= 1.5,
                "pre-C31 intermedios avg=" + intermediosAvg
                        ...
                        + "check DetailedMatchEngine professionalShotTempoMultiplier and ShotXgCalculator.baseXg.");
    ...
    private ScenarioResult runScenario(DetailedMatchEngine engine, TeamStyle style,
                                       String label, int homeOvr, int awayOvr) {
        ScenarioResult acc = new ScenarioResult();
        for (int seed = 1; seed <= N_SIMULATIONS; seed++) {
            MatchContext ctx = buildContext(style, "v33a-" + label + "-" + seed, homeOvr, awayOvr);
