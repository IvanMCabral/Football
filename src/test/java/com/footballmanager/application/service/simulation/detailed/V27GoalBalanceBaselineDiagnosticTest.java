...
class V27GoalBalanceBaselineDiagnosticTest {
    ...
    void measureBaseline_parejosOvr85x85_433() {
        ...
        for (int seed = 1; seed <= N_SIMULATIONS; seed++) {
            MatchContext ctx = buildContext("parejos-" + seed, 85, 85, "4-3-3", "4-3-3");
     ...
     */
    private void runIntermediateScenario(DetailedMatchEngine engine,
                                          String title,
        ...
        for (int seed = 1; seed <= N_SIMULATIONS; seed++) {
            MatchContext ctx = buildContext("inter-" + seed, homeOvr, awayOvr,
    ...
    void measureBaseline_desigualesOvr90x60_433v532() {
        ...
        for (int seed = 1; seed <= N_SIMULATIONS; seed++) {
            ...
            MatchContext ctx = buildContext("desiguales-" + seed, 90, 60, "4-3-3", "5-3-2");
            DetailedMatchResult result = engine.simulate(ctx, seed);
    ...
    void parejosAvgTotalGoals_inTargetBand() {
        ...
        long totalGoals = 0;
        for (int seed = 1; seed <= N_SIMULATIONS; seed++) {
            MatchContext ctx = buildContext("parejos-target-" + seed, 85, 85, "4-3-3", "4-3-3");
            DetailedMatchResult result = engine.simulate(ctx, seed);
    ...
    void parejosP4plus_inTargetBand() {
        ...
        int highScoring = 0;
        for (int seed = 1; seed <= N_SIMULATIONS; seed++) {
            MatchContext ctx = buildContext("parejos-p4-" + seed, 85, 85, "4-3-3", "4-3-3");
            DetailedMatchResult result = engine.simulate(ctx, seed);
    ...
    void desigualesAvgTotalGoals_remainsRealistic() {
        ...
        int topWins = 0;
        for (int seed = 1; seed <= N_SIMULATIONS; seed++) {
            MatchContext ctx = buildContext("desiguales-target-" + seed, 90, 60, "4-3-3", "5-3-2");
            DetailedMatchResult result = engine.simulate(ctx, seed);
            totalGoals += result.homeGoals() + result.awayGoals();
    ...
    void intermedioA_avgTotalGoals_inReducedBand() {
        ...
        long totalGoals = 0;
        for (int seed = 1; seed <= N_SIMULATIONS; seed++) {
            MatchContext ctx = buildContext("inter-a-target-" + seed, 85, 75, "4-3-3", "4-3-3");
            DetailedMatchResult result = engine.simulate(ctx, seed);
            totalGoals += result.homeGoals() + result.awayGoals();
        }
    ...
    void intermedioB_avgTotalGoals_inReducedBand() {
        ...
        long totalGoals = 0;
        for (int seed = 1; seed <= N_SIMULATIONS; seed++) {
            MatchContext ctx = buildContext("inter-b-target-" + seed, 85, 70, "4-3-3", "4-3-3");
            DetailedMatchResult result = engine.simulate(ctx, seed);
            totalGoals += result.homeGoals() + result.awayGoals();
        }
    ...
    void intermedioC_avgTotalGoals_inReducedBand() {
        ...
        long totalGoals = 0;
        for (int seed = 1; seed <= N_SIMULATIONS; seed++) {
            MatchContext ctx = buildContext("inter-c-target-" + seed, 85, 65, "4-3-3", "4-3-3");
            DetailedMatchResult result = engine.simulate(ctx, seed);
            totalGoals += result.homeGoals() + result.awayGoals();
        }
