...
class DetailedMatchEngineSecondYellowTest {
    ...
    void firstYellowDoesNotEmitRedCard() {
        ...
        long yellows = timeline.events().stream()
                .filter(e -> e.type() == DetailedMatchEventType.YELLOW_CARD)
                .count();
        long reds = timeline.events().stream()
                .filter(e -> e.type() == DetailedMatchEventType.RED_CARD)
                .count();
    ...
    void secondYellowEmitsRedCard() {
        PlayerMatchState player = freshPlayer("p2");
        ...
        DetailedMatchEvent yellowEv = timeline.events().stream()
                .filter(e -> e.type() == DetailedMatchEventType.YELLOW_CARD)
                .findFirst().orElseThrow();
        DetailedMatchEvent redEv = timeline.events().stream()
                .filter(e -> e.type() == DetailedMatchEventType.RED_CARD)
                .findFirst().orElseThrow();
    ...
    void playerRatingsCountSecondYellowAsYellowPlusRed() {
        ...
        PlayerMatchState player = PlayerMatchState.fromSessionPlayer(
                makePlayerWithId("p-yy"), "home-team");

        MatchTimeline timeline = new MatchTimeline();
        timeline.addEvent(new DetailedMatchEvent(30, DetailedMatchEventType.YELLOW_CARD,
                "home", "p-yy", "Player YY", null, null, 0.0, "first yellow"));
        timeline.addEvent(new DetailedMatchEvent(83, DetailedMatchEventType.YELLOW_CARD,
                "home", "p-yy", "Player YY", null, null, 0.0, "second yellow"));
        timeline.addEvent(new DetailedMatchEvent(83, DetailedMatchEventType.RED_CARD,
        ...
        PlayerMatchRatingDto dto = ratings.get(0);
    ...
    /**
     * Build a fresh PlayerMatchState with 0 yellows, not red, on pitch.
     */
    private PlayerMatchState freshPlayer(String id) {
        return PlayerMatchState.fromSessionPlayer(makePlayerWithId(id), "home-team");
    }
