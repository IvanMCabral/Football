...
/**
 *
 * CHANCE_CREATED path invoked {@code PlayerSelector.selectShooter(players)}
 * and {@code AssistModel.selectAssistProvider(..., null, ...)} without a
 ...
 * </ol>
 */
class DetailedMatchEngineFormationTest {
    ...
    void formationArgumentChangesShooterDistribution() {
        List<PlayerMatchState> states = toMatchStates(makeMixedLineup("sel", 1, 4, 3, 2, 1));
        ...
        PlayerSelector s442 = new PlayerSelector(new Random(42L));
        PlayerSelector s352 = new PlayerSelector(new Random(42L));
        for (int i = 0; i < 500; i++) {
    ...
    void changingHomeFormationAltersCumulativeXg() {
        ...
        DetailedMatchResult result442 = runMatch("4-4-2", "4-3-3", 42L);
        DetailedMatchResult result433 = runMatch("4-3-3", "4-3-3", 42L);
    ...
    void changingFormationProducesDifferentOutcomeAcrossSeeds() {
        ...
        for (long seed = 1L; seed <= 25L; seed++) {
            DetailedMatchResult a = runMatch("4-4-2", "4-3-3", seed);
            DetailedMatchResult b = runMatch("4-3-3", "4-3-3", seed);
    ...
    void defenderRosterChanceVolumeMultiplierRewardsAndPunishesDefensiveQuality() throws Exception {
        DefenseChannelService defenseChannelService = defenseChannelService();
    ...
    void channelSensitiveDefenderStatMakesWeakWideDefenderHurtWideShotsMore() throws Exception {
        ...
        PlayerMatchState gk = PlayerMatchState.fromSessionPlayer(
            makePlayer("chan_gk", "GK", 30, 82, 50), "teamC");
        PlayerMatchState weakLeftBack = PlayerMatchState.fromSessionPlayer(
            makePlayer("chan_lb", "DEF", 40, 25, 45), "teamC");
        PlayerMatchState strongCenterBack = PlayerMatchState.fromSessionPlayer(
            makePlayer("chan_cb", "DEF", 40, 95, 55), "teamC");
        List<PlayerMatchState> defenders = List.of(gk, weakLeftBack, strongCenterBack);
        ...
        double wideDefense = defenseChannelService.aggregateDefenderStatForLocation(
            defenders, slots, ShotLocation.PENALTY_AREA_WIDE, null, 70.0);
        double centralDefense = defenseChannelService.aggregateDefenderStatForLocation(
            defenders, slots, ShotLocation.PENALTY_AREA_CENTER, null, 70.0);
    ...
    void channelSensitiveDefenderStatDistinguishesLeftAndRightWideShots() throws Exception {
        ...
        PlayerMatchState strongRightBack = PlayerMatchState.fromSessionPlayer(
            makePlayer("lr_rb", "DEF", 40, 95, 55), "teamLR");
        PlayerMatchState neutralCenterBack = PlayerMatchState.fromSessionPlayer(
            makePlayer("lr_cb", "DEF", 40, 70, 55), "teamLR");
        List<PlayerMatchState> defenders = List.of(gk, weakLeftBack, strongRightBack, neutralCenterBack);
        ...
        ShotCoordinate leftWideShot = new ShotCoordinate(88.0, 28.0, ShotLocation.PENALTY_AREA_WIDE);
        ShotCoordinate rightWideShot = new ShotCoordinate(88.0, 72.0, ShotLocation.PENALTY_AREA_WIDE);

        double leftChannelDefense = defenseChannelService.aggregateDefenderStatForLocation(
            defenders, slots, ShotLocation.PENALTY_AREA_WIDE, leftWideShot, 70.0);
        double rightChannelDefense = defenseChannelService.aggregateDefenderStatForLocation(
            defenders, slots, ShotLocation.PENALTY_AREA_WIDE, rightWideShot, 70.0);
    ...
    void fiveThreeTwoBackFiveRaisesBothWideDefensiveChannels() {
        ...
        DetailedMatchEngine.TacticalShapeDebug shape532 = engine.debugTacticalShape(
            team, starting, List.of(), TeamStyle.BALANCED, "5-3-2", fiveThreeTwoSlots(starting));
        DetailedMatchEngine.TacticalShapeDebug shape442 = engine.debugTacticalShape(
    ...
    void leftAndRightFlankCoordinateGeneratorKeepsWideShotsOnRequestedSide() {
        ShotCoordinateGenerator generator = new ShotCoordinateGenerator();

        for (int seed = 1; seed <= 100; seed++) {
            ShotCoordinate left = generator.generateWideFlank(true, new Random(seed));
            ShotCoordinate right = generator.generateWideFlank(false, new Random(seed));

            assertTrue(left.location() == ShotLocation.PENALTY_AREA_WIDE,
                "LEFT_FLANK generated coordinate must remain PENALTY_AREA_WIDE.");
            assertTrue(right.location() == ShotLocation.PENALTY_AREA_WIDE,
    ...
    void balancedWideShotsBiasTowardOpponentWeakerDefensiveSide() throws Exception {
        ...
        TacticalShapeProfile balancedAttack = new TacticalShapeProfile(
            1.0, 1.0, 1.0,
            0.72, 0.78, 0.72,
            0.90, 0.90, 0.90);
        TacticalShapeProfile weakRightDefense = new TacticalShapeProfile(
            1.0, 1.0, 1.0,
            0.72, 0.78, 0.72,
            0.88, 0.90, 0.58);
        TacticalShapeProfile weakLeftDefense = new TacticalShapeProfile(
            1.0, 1.0, 1.0,
            0.72, 0.78, 0.72,
            0.58, 0.90, 0.88);
        ...
        for (int i = 0; i < 200; i++) {
            ShotCoordinate rightWeak = shotLocationService.generateShotCoordinate(
                ShotLocation.PENALTY_AREA_WIDE,
                TeamStyle.BALANCED,
                balancedAttack,
                weakRightDefense,
                rightRandom);
            ShotCoordinate leftWeak = shotLocationService.generateShotCoordinate(
                ShotLocation.PENALTY_AREA_WIDE,
                TeamStyle.BALANCED,
                balancedAttack,
                weakLeftDefense,
    ...
    void defenderChannelWeightChangesSmoothlyAroundLaneBoundaries() throws Exception {
        ...
        double x34 = defenseChannelService.defenderChannelWeight(ShotLocation.PENALTY_AREA_WIDE, 34.0, leftWideShot);
        double x35 = defenseChannelService.defenderChannelWeight(ShotLocation.PENALTY_AREA_WIDE, 35.0, leftWideShot);
        double x36 = defenseChannelService.defenderChannelWeight(ShotLocation.PENALTY_AREA_WIDE, 36.0, leftWideShot);
        double x64 = defenseChannelService.defenderChannelWeight(ShotLocation.PENALTY_AREA_WIDE, 64.0, leftWideShot);
        double x65 = defenseChannelService.defenderChannelWeight(ShotLocation.PENALTY_AREA_WIDE, 65.0, leftWideShot);
        double x66 = defenseChannelService.defenderChannelWeight(ShotLocation.PENALTY_AREA_WIDE, 66.0, leftWideShot);
     ...
     * {@link ShotLocation#PENALTY_AREA_WIDE} relative to a non-wingers
    ...
    void selectShotLocation_hasWingers_shiftsToWide() {
        ...
        Map<ShotLocation, Integer> counts433 = aggregateShotLocations("4-3-3", samples);
        Map<ShotLocation, Integer> counts442 = aggregateShotLocations("4-4-2", samples);

        double wideShare433 = counts433.get(ShotLocation.PENALTY_AREA_WIDE) / (double) samples;
        double wideShare442 = counts442.get(ShotLocation.PENALTY_AREA_WIDE) / (double) samples;
        ...
        assertTrue(wideShare433 > wideShare442 * 1.10,
            "4-3-3 (hasWingers) should produce at least 10% more PENALTY_AREA_WIDE shots "
                ...
                + "DetailedMatchEngine.computeLocationWeights.");
    }
    ...
    void selectShotLocation_threeFourThreeKeepsWideAttackingIdentity() {
        final int samples = 20000;
        Map<ShotLocation, Integer> counts343 = aggregateShotLocations("3-4-3", samples);
        Map<ShotLocation, Integer> counts442 = aggregateShotLocations("4-4-2", samples);

        double wideShare343 = counts343.get(ShotLocation.PENALTY_AREA_WIDE) / (double) samples;
        double wideShare442 = counts442.get(ShotLocation.PENALTY_AREA_WIDE) / (double) samples;
     ...
     * away from {@link ShotLocation#PENALTY_AREA_WIDE} relative to a
    ...
    void selectShotLocation_backThree_reducesWide() {
        final int samples = 20000;
        Map<ShotLocation, Integer> counts352 = aggregateShotLocations("3-5-2", samples);
        Map<ShotLocation, Integer> counts442 = aggregateShotLocations("4-4-2", samples);

        double wideShare352 = counts352.get(ShotLocation.PENALTY_AREA_WIDE) / (double) samples;
        double wideShare442 = counts442.get(ShotLocation.PENALTY_AREA_WIDE) / (double) samples;
    ...
    void selectShotLocation_widePlay_shiftsToWideChannel() {
        final int samples = 20000;
        Map<ShotLocation, Integer> balanced = aggregateShotLocations("4-4-2", TeamStyle.BALANCED, samples);
        Map<ShotLocation, Integer> wide = aggregateShotLocations("4-4-2", TeamStyle.WIDE_PLAY, samples);

        double balancedWideShare = balanced.get(ShotLocation.PENALTY_AREA_WIDE) / (double) samples;
        double wideShare = wide.get(ShotLocation.PENALTY_AREA_WIDE) / (double) samples;
    ...
    void selectShotLocation_centralPlay_shiftsToCentralChannel() {
        final int samples = 20000;
        Map<ShotLocation, Integer> balanced = aggregateShotLocations("4-4-2", TeamStyle.BALANCED, samples);
        Map<ShotLocation, Integer> central = aggregateShotLocations("4-4-2", TeamStyle.CENTRAL_PLAY, samples);

        double balancedCentralShare = centralShare(balanced, samples);
        double centralShare = centralShare(central, samples);
        double balancedWideShare = balanced.get(ShotLocation.PENALTY_AREA_WIDE) / (double) samples;
        double centralWideShare = central.get(ShotLocation.PENALTY_AREA_WIDE) / (double) samples;
     ...
     * {@link ShotLocation} draws. Each call uses a fresh
     ...
     * signature is
     * {@code ShotLocation selectShotLocation(TeamStyle, String, Random)}.
     */
    private Map<ShotLocation, Integer> aggregateShotLocations(String formation, int samples) {
        return aggregateShotLocations(formation, TeamStyle.BALANCED, samples);
    }

    private Map<ShotLocation, Integer> aggregateShotLocations(String formation, TeamStyle style, int samples) {
        Map<ShotLocation, Integer> counts = new EnumMap<>(ShotLocation.class);
        for (ShotLocation loc : ShotLocation.values()) {
            counts.put(loc, 0);
        }
        ShotLocationService shotLocationService = new ShotLocationService();
        TacticalShapeProfile neutral = tacticalShapeService().neutralShapeProfile();
        for (int i = 0; i < samples; i++) {
            Random r = new Random(i * 31L + 17L);
            ShotLocation loc = shotLocationService.selectShotLocation(style, formation, neutral, neutral, r);
            counts.merge(loc, 1, Integer::sum);
        }
        return counts;
    }

    private double centralShare(Map<ShotLocation, Integer> counts, int samples) {
        return (counts.get(ShotLocation.SIX_YARD_BOX)
            + counts.get(ShotLocation.PENALTY_AREA_CENTER)) / (double) samples;
    }

    // ========== Match runner ==========

    private DetailedMatchResult runMatch(String homeFormation, String awayFormation, long seed) {
        // Real Madrid-style 4-3-3 lineup: 1 GK + 4 DEF + 3 MID + 2 WINGER + 1 ATT.
    ...
    private DetailedMatchResult runMatchWithOptionalHomeSub(long seed, String subScenario) {
        ...
        List<MatchContext.ScheduledSub> manualSubs = new ArrayList<>();
        if ("upgrade-attacker".equals(subScenario)) {
            String playerOffId = homeStart.stream()
                    ...
                    .orElseThrow()
                    .getSessionPlayerId();
            manualSubs.add(new MatchContext.ScheduledSub(
    ...
    private List<PlayerMatchState> toMatchStates(List<SessionPlayer> players) {
    ...
    private int indexOf(List<PlayerMatchState> states, PlayerMatchState target) {
        for (int i = 0; i < states.size(); i++) {
    ...
    // PlayerMatchState. aggregateAttackerStat / aggregateDefenderStat now
    ...
    void tacticalPosition_changesAggregateStatEffectiveness() throws Exception {
        ...
        List<PlayerMatchState> baselineStates = new ArrayList<>();
        for (int i = 0; i < 11; i++) {
            SessionPlayer p = makePlayer("att" + i, "ATT", 100 - i, 50, 50);
            PlayerMatchState s = PlayerMatchState.fromSessionPlayer(p, "teamA");
        ...
        List<PlayerMatchState> misalignedStates = new ArrayList<>(baselineStates);
        PlayerMatchState moved = PlayerMatchState.fromSessionPlayer(
     ...
     *
     * <p>Implementation note: {@code MatchContext} accepts
     * {@code List<SessionPlayer>}, and {@code PlayerMatchState.fromSessionPlayer}
     ...
     * building pre-mutated {@code PlayerMatchState} instances with
    ...
    void lwbInMidVsCbInMid_differentXgOutcomes() throws Exception {
        ...
        List<PlayerMatchState> lineupA = new ArrayList<>();
        lineupA.add(PlayerMatchState.fromSessionPlayer(makePlayer("gk", "GK", 30, 80, 50), "teamL"));
        for (int i = 0; i < 4; i++) {
            lineupA.add(PlayerMatchState.fromSessionPlayer(
        ...
        PlayerMatchState lwb = PlayerMatchState.fromSessionPlayer(
                makePlayer("lwb_var", "WINGER", 95, 50, 80), "teamL");
        lwb.setPosition("MID");  // WINGER-natural -> MID tactical (effectiveness 0.95)
        lineupA.add(lwb);

        List<PlayerMatchState> lineupB = new ArrayList<>();
        lineupB.add(PlayerMatchState.fromSessionPlayer(makePlayer("gk", "GK", 30, 80, 50), "teamC"));
        for (int i = 0; i < 4; i++) {
            lineupB.add(PlayerMatchState.fromSessionPlayer(
        ...
        PlayerMatchState cb = PlayerMatchState.fromSessionPlayer(
    ...
    void backyardCompat_noTacticalPosition_unchangedBehavior() throws Exception {
        ...
        for (int i = 0; i < 1; i++) {
            ...
            states.add(PlayerMatchState.fromSessionPlayer(p, "teamX"));
        }
        for (int i = 0; i < 4; i++) {
     ...
     * {@code SessionPlayer} via {@code MatchContext.homeStartingPlayers}
     * and builds {@code PlayerMatchState} via {@code fromSessionPlayer},
     ...
     * exercisable through MatchContext — they require direct construction
     * of {@code PlayerMatchState} + {@code setPosition}, which is what
    ...
    void fiveFormationsProbe_effectivenessChangesOutcomes() {
        ...
        DetailedMatchResult r442 = runMatchWithLineup("4-4-2",   "4-4-2", seed, homeBase, awayStart);
        DetailedMatchResult r352 = runMatchWithLineup("3-5-2",   "4-4-2", seed, homeBase, awayStart);
        DetailedMatchResult r532 = runMatchWithLineup("5-3-2",   "4-4-2", seed, homeBase, awayStart);
        DetailedMatchResult r433 = runMatchWithLineup("4-3-3",   "4-4-2", seed, homeBase, awayStart);
        DetailedMatchResult r4411 = runMatchWithLineup("4-4-1-1","4-4-2", seed, homeBase, awayStart);
    ...
    void lwbInAtt_highPenalty() throws Exception {
        ...
        List<PlayerMatchState> naturalStates = new ArrayList<>();
        naturalStates.add(PlayerMatchState.fromSessionPlayer(makePlayer("gk0", "GK", 30, 80, 50), "teamZ"));
        for (int i = 0; i < 3; i++) {
            int attack = (i == 0) ? 95 : 50;  // first DEF is a "tweener" with attack 95
            naturalStates.add(PlayerMatchState.fromSessionPlayer(
        ...
        List<PlayerMatchState> misalignedStates = new ArrayList<>(naturalStates);
        PlayerMatchState movedDef = PlayerMatchState.fromSessionPlayer(
    ...
    void visualForwardMoveOfMidfielderRaisesEngineAttackInput() throws Exception {
        List<PlayerMatchState> states = new ArrayList<>();
        states.add(PlayerMatchState.fromSessionPlayer(makePlayer("gk0", "GK", 30, 80, 50), "teamP"));
        for (int i = 0; i < 4; i++) {
            states.add(PlayerMatchState.fromSessionPlayer(
        ...
        states.add(PlayerMatchState.fromSessionPlayer(makePlayer("att0", "ATT", 86, 50, 82), "teamP"));
        states.add(PlayerMatchState.fromSessionPlayer(makePlayer("att1", "ATT", 85, 50, 82), "teamP"));
    ...
    void visualForwardMoveOfAttackerRaisesEngineAttackInput() throws Exception {
        List<PlayerMatchState> states = new ArrayList<>();
        states.add(PlayerMatchState.fromSessionPlayer(makePlayer("gk0", "GK", 30, 80, 50), "teamAF"));
        for (int i = 0; i < 4; i++) {
            states.add(PlayerMatchState.fromSessionPlayer(
        ...
        states.add(PlayerMatchState.fromSessionPlayer(makePlayer("att0", "ATT", 88, 50, 82), "teamAF"));
        states.add(PlayerMatchState.fromSessionPlayer(makePlayer("att1", "ATT", 86, 50, 82), "teamAF"));
    ...
    void sameVisualAttackingSlotPlayerSwapChangesEngineAttackInput() throws Exception {
        ...
        List<PlayerMatchState> forcedDefenderStates = toMatchStates(strongNaturalLineup);
        PlayerMatchState defenderInAttackingSlot = PlayerMatchState.fromSessionPlayer(
    ...
    void scheduledSubstitutionChangesAttackVolumeOnlyAfterEffectiveMinute() throws Exception {
        ...
        TeamMatchState upgradeState = TeamMatchState.create(
                team, starting, benchWithUpgrade, TeamStyle.BALANCED, Map.of());
        TeamMatchState downgradeState = TeamMatchState.create(
                team, starting, benchWithDowngrade, TeamStyle.BALANCED, Map.of());

        List<MatchContext.ScheduledSub> upgradeSubstitution = List.of(
                new MatchContext.ScheduledSub(
                        HOME_UUID, playerOffId, upgradePlayerOnId, 60));
        List<MatchContext.ScheduledSub> downgradeSubstitution = List.of(
                new MatchContext.ScheduledSub(
    ...
    void fullMatchScheduledSubstitutionAltersOutcomeAcrossSeeds() {
        ...
        for (long seed = 1L; seed <= 25L; seed++) {
            DetailedMatchResult baseline = runMatchWithOptionalHomeSub(seed, null);
            DetailedMatchResult upgrade = runMatchWithOptionalHomeSub(seed, "upgrade-attacker");
     ...
     */
    private double invokeAggregateAttackerStat(List<PlayerMatchState> players, String formation)
            throws Exception {
     ...
     */
    private double invokeAggregateDefenderStat(List<PlayerMatchState> players)
            throws Exception {
    ...
    private double invokeAttackVolumeForMovedMidfielder(
        ...
        TeamMatchState state = TeamMatchState.create(team, starting, List.of(), TeamStyle.BALANCED, slots);
    ...
    private double invokeShapeMetric(
        ...
        TacticalShapeProfile profile = formation == null
    ...
    private TacticalPositionService tacticalPositionService() {
        return new TacticalPositionService();
    }

    private TacticalEffectivenessService tacticalEffectivenessService() {
        return new TacticalEffectivenessService(tacticalPositionService());
    }

    private AttackContributionService attackContributionService() {
        return new AttackContributionService(tacticalEffectivenessService());
    }

    private DefenseChannelService defenseChannelService() {
        return new DefenseChannelService(tacticalPositionService(), tacticalEffectivenessService());
    }

    private TacticalShapeService tacticalShapeService() {
        return new TacticalShapeService(tacticalPositionService(), tacticalEffectivenessService());
    }
     ...
     * through this path because {@code MatchContext} takes
     ...
     * 1, 2, 3, 5).
     */
    private DetailedMatchResult runMatchWithLineup(
    ...
    void simulation_3_5_2_CDM_runsWithoutError() {
        DetailedMatchResult result = runMatch("3-5-2-CDM", "4-4-2", 42L);
    ...
    void simulation_5_4_1_runsWithoutError() {
        DetailedMatchResult result = runMatch("5-4-1", "4-4-2", 42L);
    ...
    void simulation_3_4_1_2_runsWithoutError() {
        DetailedMatchResult result = runMatch("3-4-1-2", "4-4-2", 42L);
    ...
    void simulation_4_2_2_2_runsWithoutError() {
        DetailedMatchResult result = runMatch("4-2-2-2", "4-4-2", 42L);
    ...
    void simulation_4_1_2_3_runsWithoutError() {
        DetailedMatchResult result = runMatch("4-1-2-3", "4-4-2", 42L);
     ...
     * </ul>
     */
    private void assertValidMatchResult(DetailedMatchResult result, String formation) {
