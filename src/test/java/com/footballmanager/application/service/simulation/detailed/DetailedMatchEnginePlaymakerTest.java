 ...
 * </ol>
 */
class DetailedMatchEnginePlaymakerTest {
    ...
    void playmakerAdj_doesNotCapAt1() throws Exception {
        ...
        // El clamp lo pone ShotQuality (max 100), no este metodo.
        double base = 0.9;
        double adjusted = invokePlaymakerAdj(base, 99);
        assertEquals(0.9 * 1.495, adjusted, 0.0001,
                "PLAYMAKER no debe capar a 1.0 — el clamp lo hace ShotQuality");
    }
    ...
    void fullMatch_playmakerHigh_producesMoreGoalsOrMoreXgThanBaseline() {
        ...
        MatchContext baseline = buildContextWithPlaymaker("playmaker-base", -1);
        MatchContext treatment = buildContextWithPlaymaker("playmaker-high", 99);
    ...
    void fullMatch_noPlaymakerSkill_preservesV25D33Baseline() {
        ...
        MatchContext baseline = buildContextWithPlaymaker("no-playmaker", -1);  // sentinel: sin PLAYMAKER
    ...
    private double invokePlaymakerAdj(double base, int playmakerSkill) throws Exception {
        return ShotAttemptService.playmakerAdjustedAssistQuality(base, playmakerSkill);
    }

    // ========== Fixture builders ==========

    /**
     * Construye un MatchContext con 11 jugadores por equipo. El primer
     ...
     */
    private MatchContext buildContextWithPlaymaker(String matchId, int playmakerSkill) {
