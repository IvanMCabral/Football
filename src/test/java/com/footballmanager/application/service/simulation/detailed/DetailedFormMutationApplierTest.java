 ...
 *
 * The applier delegates to PlayerRatingsAssembler which computes ratings
 * from CareerSave starting XI + MatchTimeline. Tests must set up
 * teamStarting11 to control which players get ratings.
 */
class FormMutationApplierTest {

    private final FormMutationApplier applier = new FormMutationApplier();
    ...
    private DetailedMatchResult resultWithTimeline(DetailedMatchEvent... events) {
    ...
    void nullCareer_returnsZero() {
        DetailedMatchResult res = resultWithTimeline();
        CareerMutationPolicy p = policy(true, false, false, false, true);
    ...
    void disabledPolicy_noMutation() {
        CareerSave career = careerWithPlayerInStartingXI("p1", 50);
        DetailedMatchResult res = resultWithTimeline(goalEvent("p1", 30));
    ...
    void unknownPlayer_skipped() {
        ...
        DetailedMatchResult res = resultWithTimeline(goalEvent("unknown", 30));
        CareerMutationPolicy p = policy(true, false, false, false, true);
    ...
    void blankPlayerId_skipped() {
        ...
        DetailedMatchResult res = resultWithTimeline(
                new DetailedMatchEvent(30, DetailedMatchEventType.GOAL, "team-A", "  ", "Scorer", null, null, 0.35, "Goal")
        );
    ...
    void ratingGood_increasesForm() {
        ...
        DetailedMatchResult res = resultWithTimeline(goalEvent("p1", 30), goalEvent("p1", 60));
        CareerMutationPolicy p = policy(true, false, false, false, true);
        assertEquals(1, applier.applyForm(career, res, p));
    ...
    void ratingAcceptable_increasesForm() {
        ...
        DetailedMatchResult res = resultWithTimeline(
                new DetailedMatchEvent(30, DetailedMatchEventType.GOAL, "team-A", "p1", "Scorer", null, null, 0.35, "Goal")
        );
        CareerMutationPolicy p = policy(true, false, false, false, true);
        assertEquals(1, applier.applyForm(career, res, p));
        assertEquals(51, career.getPlayerManager().getSessionPlayer("p1").getForm());
    ...
    void ratingPoor_decreasesForm() {
        ...
        DetailedMatchResult res = resultWithTimeline(
                new DetailedMatchEvent(30, DetailedMatchEventType.YELLOW_CARD, "team-A", "p1", "Player", null, null, 0.0, "Card"),
                new DetailedMatchEvent(45, DetailedMatchEventType.FOUL, "team-A", "p1", "Player", null, null, 0.0, "Foul")
        );
        CareerMutationPolicy p = policy(true, false, false, false, true);
        assertEquals(1, applier.applyForm(career, res, p));
        assertEquals(50, career.getPlayerManager().getSessionPlayer("p1").getForm());
    ...
    void ratingVeryPoor_decreasesForm() {
        ...
        DetailedMatchResult res = resultWithTimeline(
                new DetailedMatchEvent(30, DetailedMatchEventType.RED_CARD, "team-A", "p1", "Player", null, null, 0.0, "Red")
        );
        CareerMutationPolicy p = policy(true, false, false, false, true);
        assertEquals(1, applier.applyForm(career, res, p));
    ...
    void formClampedAtMax() {
        ...
        DetailedMatchResult res = resultWithTimeline(goalEvent("p1", 30), goalEvent("p1", 60), goalEvent("p1", 75));
        CareerMutationPolicy p = policy(true, false, false, false, true);
        assertEquals(1, applier.applyForm(career, res, p));
    ...
    void nullExistingForm_defaultsNeutral() {
        ...
        DetailedMatchResult res = resultWithTimeline(goalEvent("p1", 30));
        CareerMutationPolicy policy = policy(true, false, false, false, true);
    ...
    void multiplePlayers_independentChanges() {
        ...
        DetailedMatchResult res = resultWithTimeline(
                goalEvent("p1", 30),
                new DetailedMatchEvent(45, DetailedMatchEventType.RED_CARD, "team-B", "p2", "Player", null, null, 0.0, "Red")
        );
        CareerMutationPolicy p = policy(true, false, false, false, true);
