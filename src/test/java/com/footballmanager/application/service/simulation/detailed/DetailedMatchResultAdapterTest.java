...
class DetailedMatchResultAdapterTest {
    ...
    void mapsAggregateFieldsOnly() {
        MatchContext ctx = buildContext("match-adapter-1", 75, 75);
        DetailedMatchEngine engine = new DetailedMatchEngine();
        DetailedMatchResult detailed = engine.simulate(ctx, 123L);

        // Adapt to MatchResultData
        MatchFixture.MatchResultData data =
                DetailedMatchResultAdapter.toMatchResultData(detailed);
    ...
    void adapterRejectsNullResult() {
        assertThrows(IllegalArgumentException.class, () ->
                DetailedMatchResultAdapter.toMatchResultData(null));
    }
