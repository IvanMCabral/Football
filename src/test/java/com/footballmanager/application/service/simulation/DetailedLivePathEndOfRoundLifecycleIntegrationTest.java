...
class DetailedLivePathEndOfRoundLifecycleIntegrationTest {
    ...
    void preExistingSuspendedInParticipatedPlayerIds_decrementFires() {
        ...
        LiveRoundMutationTracking tracking = new LiveRoundMutationTracking(1, 1);
        tracking.preRoundSuspendedPlayerIds.add(p1);
        // Simulate: detailed match engine emitted events for p1 (they are in starting XI),
    ...
    void applyLiveMatchCareerMutations_excludesSuspendedFromParticipated() {
        ...
        timeline.addEvent(new DetailedMatchEvent(
            10, DetailedMatchEventType.GOAL,
            HOME, suspId, "Suspended Scorer", null, null, 0.0, "Susp goal"));
        timeline.addEvent(new DetailedMatchEvent(
            20, DetailedMatchEventType.GOAL,
            HOME, healthyId, "Healthy Scorer", null, null, 0.0, "Healthy goal"));

        DetailedMatchResult detailedResult = DetailedMatchResult.builder()
    ...
    private static class FakeStoragePort implements DetailedMatchStoragePort {
        @Override
        public Mono<Void> save(String careerId, com.footballmanager.application.service.simulation.detailed.DetailedMatchData detail) { return Mono.empty(); }
        @Override
        public Mono<java.util.Optional<com.footballmanager.application.service.simulation.detailed.DetailedMatchData>> findByMatchId(String careerId, String matchId) {
            return Mono.just(java.util.Optional.empty());}
        @Override
        public Flux<com.footballmanager.application.service.simulation.detailed.DetailedMatchData> findByCareerId(String careerId) {
