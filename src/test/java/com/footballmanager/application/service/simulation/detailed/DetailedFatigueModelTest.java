...
class FatigueModelTest {
    ...
    private final FatigueModel calc = new FatigueModel();
    ...
    void staminaNeverBelowZero() {
        PlayerMatchState player = makePlayer("stam-zero", 50, 100);
        // Apply huge drain
    ...
    void staminaNeverAboveStartingValue() {
        ...
        PlayerMatchState player = makePlayer("stam-over", 80, 100);
    ...
    void zeroDrainDoesNothing() {
        PlayerMatchState player = makePlayer("zero-drain", 50, 100);
    ...
    void negativeDrainDoesNothing() {
        PlayerMatchState player = makePlayer("neg-drain", 50, 100);
    ...
    void fatigueReducesQuality() {
        ...
        PlayerMatchState fresh = makePlayerWithStamina("fresh", 100);
        assertEquals(1.00, calc.applyFatigueToQuality(1.0, fresh), 0.001);

        // Exhausted player: 50% penalty
        PlayerMatchState exhausted = makePlayerWithStamina("exhausted", 10);
        double result = calc.applyFatigueToQuality(1.0, exhausted);
        assertEquals(0.50, result, 0.001);

        // Medium stamina: 85%
        PlayerMatchState medium = makePlayerWithStamina("medium", 50);
    ...
    void applyFatigueToQualityHandlesNonFinite() {
        PlayerMatchState p = makePlayerWithStamina("finite", 75);
    ...
    private PlayerMatchState makePlayer(String id, int ovr, int energy) {
    ...
    private PlayerMatchState makePlayerWithStamina(String id, int startingStamina) {
        SessionPlayer sp = SessionPlayer.custom(
                id, 25, "MID",
                70, 70, 70, 70, 70, 70,
