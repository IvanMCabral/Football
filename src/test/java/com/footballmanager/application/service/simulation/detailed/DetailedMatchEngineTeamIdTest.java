 ...
 *
 */
class DetailedMatchEngineTeamIdTest {
    ...
    void timelineEventTeamIdsAreRealUuidsNotHomeAway() {
        ...
        List<DetailedMatchEvent> events = result.timeline().events();
     ...
     * MISS, BLOCK}. The Stats summary in the detailed match detail page reads homeShots/awayShots;
    ...
    void homeShotsPlusAwayShotsEqualsTimelineShotCount() {
        ...
        DetailedMatchResult result = null;
        ...
        int timelineHomeShots = (int) result.timeline().events().stream()
                .filter(e -> e.teamId().equals(HOME_UUID))
                .filter(e -> e.type() == DetailedMatchEventType.GOAL
                        || e.type() == DetailedMatchEventType.SHOT_ON_TARGET
    ...
    private MatchContext buildContext(String matchId, String homeTeamId, String awayTeamId) {
