...
class TimelineSnapshotBuilderTest {
    ...
    private static DetailedMatchEventDto ev(int minute, String type, String teamId, double xg) {
        return new DetailedMatchEventDto(minute, type, teamId, "p1", "Player", null, null, xg, "", null);
    }

    private static DetailedMatchData detail(List<DetailedMatchEventDto> events) {
    ...
    void emptyTimelineAtMinuteZero() {
        DetailedMatchData d = detail(List.of());

        TimelineSnapshot snap = TimelineSnapshotBuilder.build(d, 0);
    ...
    void minuteZero_excludesAllEvents() {
        ...
        // All events have minute >= 1 (DetailedMatchEventDto rejects minute=0),
    ...
    void minuteFortyFive_filtersEventsUpToAndIncluding() {
        ...
        TimelineSnapshot snap = TimelineSnapshotBuilder.build(d, 45);
    ...
    void minuteOneHundredThirty_includesAllEvents() {
        ...
        TimelineSnapshot snap = TimelineSnapshotBuilder.build(d, 130);
    ...
    void goalContributesToBothGoalsAndXg() {
        ...
        TimelineSnapshot snap = TimelineSnapshotBuilder.build(d, 90);
    ...
    void mixedScenarioRealisticMatch() {
        ...
        List<DetailedMatchEventDto> events = new ArrayList<>();
        ...
        DetailedMatchData d = detail(events);

        // Snapshot at minute 50: should include events up to min 50 only
        TimelineSnapshot s50 = TimelineSnapshotBuilder.build(d, 50);
        ...
        TimelineSnapshot sFull = TimelineSnapshotBuilder.build(d, 130);
