...
class MatchComparisonServiceTest {
    ...
    @Mock
    private DetailedMatchStoragePort detailStoragePort;
    @Mock
    ...
    void getComparison_noSubs_returnsIdenticalBaselineAndLive() {
        ...
        MatchContext context = buildContext(MATCH_ID);
        ...
        DetailedMatchResult liveResult = engine.simulate(context, random);
        DetailedMatchData live = DetailedMatchData.fromResult(
    ...
    void getComparison_oneSub_producesDifferentBaselineFromLive() {
        ...
        MatchContext liveContext = context.withManualSubstitution(
                "home-id", "home-id-starter-3", "home-id-bench-0", 60);
        DetailedMatchEngine engine = new DetailedMatchEngine();
        CachingRandomWrapper liveRandom = new CachingRandomWrapper(SEED);
        DetailedMatchResult liveResult = engine.simulate(liveContext, liveRandom);
        DetailedMatchData live = DetailedMatchData.fromResult(
                CAREER_ID, 1, 5, "Home FC", "Away FC", liveResult, List.of());
    ...
    void getComparison_twoSubs_producesValidComparison() {
        ...
        MatchContext liveContext = context
        ...
        DetailedMatchEngine engine = new DetailedMatchEngine();
        DetailedMatchResult liveResult = engine.simulate(liveContext,
    ...
    /**
     * Mirrors {@code LiveSessionTest.buildContext()} so the test data
     * shape matches what the production code uses.
     */
    private MatchContext buildContext(String matchId) {
    ...
    private DetailedMatchData sampleLive() {
        return new DetailedMatchData(
                ...
                10, 6, 55, 45,
                List.of(), List.of(),
                "Sample", "DETAILED_MATCH", 1, Instant.now(), null, null);
    }
}
