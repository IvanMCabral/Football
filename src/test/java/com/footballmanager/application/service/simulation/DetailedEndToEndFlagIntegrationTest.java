...
class V24EndToEndFlagIntegrationTest {
    ...
    void v24EnabledPersistDisabledProducesAggregateOnly() {
        ...
        // detailed match path used — possession should sum to 100
    ...
    /**
     * All three flags enabled: detailed match path, persistence, and API flag (read-side).
     * Simulation uses detailed match path, saves detail, and round completes.
    ...
    void v24TakesPrecedenceOverclassicWhenBothEnabled() {
        ...
        // classic path also not used (realEngine path is classic)
        assertTrue(fakeStorage.saveCalled, "detailed match path should be used (wins over classic)");
        assertNotNull(career.getTournamentState().getFixtures().get(0).getResult(),
                "Round should complete via detailed match path");
    }
    ...
    /**
     * detailed match path writes aggregate (goals, possession, shots) only.
    ...
    void matchResultDataSchemaStillSixFields() {
        ...
        // Possession sums to 100 in detailed match path
        assertEquals(100, result.homePossession + result.awayPossession,
    ...
    void noCareerStateMutationAfterV24Simulation() {
        ...
        assertEquals(homePlayerEnergyBefore, homePlayerEnergyAfter,
                "Player energy should not change after detailed match simulation");
        assertEquals(homeFormationBefore, homeFormationAfter,
                "Team formation should not change after detailed match simulation");
        assertNotNull(career.getTournamentState().getFixtures().get(0).getResult(),
    ...
    private static class ThrowingStoragePort implements DetailedMatchStoragePort {
        @Override
        public Mono<Void> save(String careerId, DetailedMatchData detail) { return Mono.error(new RuntimeException("Redis connection failed")); }
