...
| Detailed package naming | Production code uses `application.service.simulation.detailed`, not `simulation.detailed`. |
...
| Formation naming | `FormationParser.FormationShape` replaces `DetailedFormation`. |
...
| Classic fallback | Former Classic path is represented as the classic engine path and remains available because it is still a configured fallback. |
| Compatibility boundary | Old config keys and persisted `engineType: "Detailed"` are retained only as explicit compatibility contracts. |
...
The only remaining Detailed/Classic strings in production are compatibility/property/persisted-contract references or explanatory text around old save formats. They are not active class/package/method names for the detailed simulation domain.
